...
class Main(QtWidgets.QMainWindow, MainWindow_main):
    ...
    def sn_cal(self):
        ...
        F_o = 1 - 0.22 * (log10(rz) ** 0.64) * log10(sigma_b) + 0.45 * (log10(rz) ** 0.53)
        ...
        u = 1 / (M + 1) * sigm_wk / sigma_b
        a = (1 + R) / (1 - R) * sigm_wk / sigma_b
        p = (1 / (M + 1) - 1 + u ** 2) / (u ** 2 - u)
        if a == 0:
            Fm = 1
        else:
        ...
        with open('SN_Curve.txt', 'w') as f:
            ...
            f.write('*'*30+'\n')
            f.write('Calculation of the SN curve according to the GL2010 5.B.3.1 and 5.B.3.2.\n')
            ...
            if mat == 'GGG':
                f.write(str(round((rp / gamma_m - sigma_d) / (1 - M), 3))+'/'+str(round(M*(rp / gamma_m - sigma_d) /
                                                                                        (M - 1) + sigma_d, 3))+'\n')
            else:
                f.write('')
            f.write(str(round(sigma_p, 3))+'/'+str(round(sigma_p, 3))+'\n')
            f.write('0'+'/'+str(round(sigma_d, 3))+'\n')
            f.write(str(round(sigma_d / (M - 1), 3)) + '/' + str(round(-sigma_d / (M - 1), 3))+'\n')
            f.write(str(round(-rp/gamma_m, 3))+'/'+'0\n')
            f.close()
        # Plot of the SN curve
        x = [0, N_1, N_D, N_e]
        y = [sigma_1, sigma_1, sigma_d, sigma_e]
        plt.loglog(x, y, lw=2, marker='*')
        plt.xlabel('Cycle Numbers')
        plt.ylabel('Stress Amplitude/MPa')
        plt.xlim(10, 10 ** 8)
        plt.yticks([10, 100, 1000])
        plt.annotate(s='(%.1e,%.2f)' % (N_1, sigma_1), xy=(N_1, sigma_1))
