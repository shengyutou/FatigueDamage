...
def signal_filter(data, order=8, cutoff=0.3):
    ...
    return array(
        [filtfilt(b, a, data[:, ifl]) for ifl in range(data.shape[1])]
    ).T


def loads_read(file_loc, ndigits=2, tf_filter=True):
    """
        ...
        for key in load_file:
            ...
            if s_method == "max_principle":
                stress_r[key] = (stress_c[key][:, 0] + stress_c[key][:, 1]).reshape(
                    ...
                    (stress_c[key][:, 0] - stress_c[key][:, 1]).reshape([locnum, 1])
                    ** 2
                    / 4
...
if __name__ == "__main__":
    ...
    start = time.time()
    unit_load = test.unit_stress("./Factor", 15)
    unit_load_result = {}
    for i in range(len(unit_load)):
        unit_load_result[unit_load[i, 0]] = unit_load[i, 1:]
    del unit_load
    end = time.time()
    ...
    for node in list(unit_load_result.keys())[:1]:
        stress_s = test.stress_combine(load_all, unit_load_result[node], s_method="1")
        for key in stress_s:
